...
from dataloaders.dataset import TestReasoningDataset, TestReferDataset, collate_fn_test
from model.SESAME import load_pretrained_model_SESAME
from model.READ import load_pretrained_model_READ
from model.UGround import load_pretrained_model_UGround
...
def parse_args(args):
    ...
    parser.add_argument("--version", default="tsunghanwu/SESAME")
    parser.add_argument("--vis_save_path", default="./save_directory", type=str)
    parser.add_argument("--image_size", default=1024, type=int, help="image size")
    parser.add_argument("--model_max_length", default=512, type=int)
    parser.add_argument("--val_dataset", default="fprefcoco|val", type=str)
    ...
    parser.add_argument("--model_key", default="SESAME", type=str, help="Model key for ModelFactory")
    parser.add_argument(
        "--conv_type",
        default="llava_v1",
        type=str,
...
def inference(args):
    ...
    load_model_functions = {
        'SESAME': lambda: load_pretrained_model_SESAME(model_path=args.version, vision_tower=args.vision_tower, model_max_length=args.model_max_length),
        'READ': lambda: load_pretrained_model_READ(model_path=args.version, vision_tower=args.vision_tower, model_max_length=args.model_max_length),
        'UGround': lambda: load_pretrained_model_UGround(model_path=args.version, vision_tower=args.vision_tower, model_max_length=args.model_max_length, **kwargs),
    }
    (
        tokenizer,
        segmentation_lmm,
        vision_tower,
        context_len,
    ) = load_model_functions[args.model_key]()
    segmentation_lmm = torch.compile(segmentation_lmm, mode="reduce-overhead")

    # for eval only
    tokenizer.padding_side = "left"
    reason_seg_dataset = ["ReasonSeg", "fpReasonSeg"]
    refer_seg_dataset = [
    ...
    splits = args.val_dataset.split('|')
    assert len(splits) == 2, "val_dataset should be in the format of dataset|split"
    ds, split = splits
    if ds in reason_seg_dataset:
    ...
    for input_dict in tqdm.tqdm(test_loader):
        ...
        for n in range(num_batch):
            ...
            clip_resize_list = input_dict["clip_resize_list"] * real_batch_size
            # Model evaluation
            with torch.inference_mode():
                output_ids, pred_masks_batch, object_presence, _ = segmentation_lmm.evaluate(
