...
def parse_args(args):
    ...
    parser.add_argument(
        "--dataset", default="sem_seg||fp_sem_seg||refer_seg||fp_refer_seg||neg_refer_seg||correct_refer_seg||vqa||reason_seg||fp_reason_seg||reason_seg_plus||multi_reason_seg", type=str
    )
    ...
    parser.add_argument("--latent_token_num", default=7, type=int)
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Evaluate Referring Expression Comprehension (REC) by converting masks to bounding boxes and "
             "computing bbox IoU > 0.5 accuracy on the same RES datasets.",
    )
    parser.add_argument(
        "--conv_type",
        default="llava_v1",
...
def main(args):
    ...
    if args.latent_token_num > 0:
        new_tokens = ["[LATENT{}]".format(i) for i in range(args.latent_token_num)]
        num_added_tokens = tokenizer.add_tokens(new_tokens)
        args.latent_token_idx = [tokenizer(token, add_special_tokens=False).input_ids[0] for token in new_tokens]
        LATENT_PLACEHOLDER_STR =  ' '.join(new_tokens) 
    else:
        args.latent_token_idx = None
        LATENT_PLACEHOLDER_STR = None
    ...
    model_args = {
        ...
        "latent_token_idx": args.latent_token_idx,
        "vision_pretrained": args.vision_pretrained,
        "vision_tower": args.vision_tower,
        "use_mm_start_end": args.use_mm_start_end,
        "seg_token_num": args.seg_token_num,
        "latent_token_num": args.latent_token_num,
    ...
    if not args.eval_only and hasattr(model, "ppm") and model.ppm is not None:
    ...
    trainable_list = ["lm_head", "embed_tokens", "mask_decoder", "text_hidden_fcs", "latent_hidden_fcs", "prompt_encoder", "ppm"]
    if args.resize_vision_tower_size != 224:
    ...
    model_engine, optimizer, train_loader, scheduler = deepspeed.initialize(
        ...
        collate_fn=partial(
            ...
            use_mm_start_end=args.use_mm_start_end,
            local_rank=args.local_rank,
            LATENT_PLACEHOLDER_STR=LATENT_PLACEHOLDER_STR
        ),
        config=ds_config,
    )
    ...
    if args.eval_only:
        ...
        for loader, dataset_name in zip(val_loader, val_dataset_names):
            ...
            if args.eval_rec and dataset_type != 'MultiReasonSeg':
                eval_rec(loader, model_engine, 0, writer, args, logger, dataset_name)
                continue
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.no_eval == False:
            if args.eval_rec:
                # For REC: use bbox IoU>0.5 accuracy as the primary metric for checkpointing.
                rec_acc, rec_bbox_iou = eval_rec(
                    val_loader, model_engine, epoch, writer, args, logger, val_dataset_names
                )
                giou, ciou = rec_bbox_iou, rec_acc
            elif any('grefcoco' in name for name in args.val_dataset_names):
...
@torch.no_grad()
def eval_rec(val_loader, model_engine, epoch, writer, args, logger, dataset_name, iou_thr: float = 0.5):
    """
    Referring Expression Comprehension (REC) evaluation.
    Since RES datasets provide masks, we compute bounding boxes from masks:
    - Convert predicted / GT masks to bounding boxes
    - A prediction is correct if bbox IoU > iou_thr
    """

    def _mask_to_box_xyxy(mask_2d: torch.Tensor):
        # Returns (x1, y1, x2, y2) with x2/y2 exclusive max; None if empty.
    ...
    def _box_iou_xyxy(a, b) -> torch.Tensor:
        ax1, ay1, ax2, ay2 = a
        bx1, by1, bx2, by2 = b
    ...
    acc_meter = AverageMeter("REC@0.5", ":6.3f")
    bbox_iou_meter = AverageMeter("REC_BBoxIoU", ":6.3f")

    loaders = val_loader if isinstance(val_loader, list) else [val_loader]
    name_for_print = "||".join(dataset_name) if isinstance(dataset_name, (list, tuple)) else dataset_name

    for _loader in loaders:
        for input_dict in tqdm.tqdm(_loader):
            torch.cuda.empty_cache()
            ...
            gt_masks = output_dict["gt_masks"][0]

            # Optional: if model emits [REJ] for a slot, force predicted mask to empty.
            pred_is_rej = None
            try:
                ...
                if len(seg_or_rej_index) == len(gt_masks):
                    pred_is_rej = (output_ids[seg_or_rej_index] == args.rej_token_idx)
            except Exception:
                pred_is_rej = None

            pred_bin = (pred_masks > 0)
            gt_bin = (gt_masks > 0) & (gt_masks != 255)
            assert len(pred_bin) == len(gt_bin)

            for i, (pred_m, gt_m) in enumerate(zip(pred_bin, gt_bin)):
                if pred_is_rej is not None and bool(pred_is_rej[i].item()):
                    pred_m = torch.zeros_like(pred_m, dtype=torch.bool)

                gt_empty = gt_m.sum().item() < 1
                pred_empty = pred_m.sum().item() < 1

                if gt_empty and pred_empty:
                    iou = torch.tensor(1.0, device=pred_m.device)
                    correct = 1.0
                elif gt_empty != pred_empty:
                    iou = torch.tensor(0.0, device=pred_m.device)
                    correct = 0.0
                else:
                    gt_box = _mask_to_box_xyxy(gt_m)
                    pred_box = _mask_to_box_xyxy(pred_m)
                    iou = _box_iou_xyxy(pred_box, gt_box)
                    correct = float((iou > iou_thr).item())

                bbox_iou_meter.update(float(iou.item()))
                acc_meter.update(correct)

    acc_meter.all_reduce()
    bbox_iou_meter.all_reduce()

    rec_acc = float(acc_meter.avg)
    rec_bbox_iou = float(bbox_iou_meter.avg)
    if args.local_rank == 0:
        writer.add_scalar("val/rec_acc@0.5", rec_acc, model_engine.global_steps)
        writer.add_scalar("val/rec_bbox_iou", rec_bbox_iou, model_engine.global_steps)
        print("{}, {}, epoch: {}, rec_acc@0.5: {:.4f}, rec_bbox_iou: {:.4f}".format(
            args.model_key, name_for_print, epoch, rec_acc, rec_bbox_iou
        ))

    return rec_acc, rec_bbox_iou
