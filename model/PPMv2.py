...
class PolicyPromptedMasking(nn.Module):
    ...
    def recurrent_unrolled(self, hidden_states, input_ids, seg_token_mask, latent_token_num, num_patches):        
        ...
        token_idx = (torch.arange(T, device=device).unsqueeze(0).expand(B, -1)[seg_token_mask_bool]) # [N_seg]
        latent_offsets = torch.arange(-latent_token_num, 0, device=device) # [-K, ..., -1]
        latent_token_idx = token_idx.unsqueeze(1) + latent_offsets # [N_seg, K]
        latent_token_embeds = hidden_states.permute(0, 2, 1, 3)[batch_idx.unsqueeze(1), latent_token_idx] # [N_seg, K, L, D]
        latent_token_embeds = latent_token_embeds.permute(0, 2, 1, 3) # [N_seg, L, K, D]

        return seg_token_embeds, seg_image_token_embeds, latent_token_embeds
    ...
    def quick_select(
        ...
        token_idx = torch.arange(T, device=device).unsqueeze(0).expand(B, -1)
        seg_token_idx = token_idx[seg_token_mask.bool()]  # [N_seg]
        latent_offsets = torch.arange(-latent_token_num, 0, device=device)  # [K]
        latent_token_idx = seg_token_idx.unsqueeze(1) + latent_offsets  # [N_seg, K]
        latent_token_embeds = hidden_states[batch_idx.unsqueeze(1), latent_token_idx]  # [N_seg, K, D]
        
        return seg_token_embeds, seg_image_token_embeds, seg_token_embeds, latent_token_embeds
        # similarity = torch.einsum("sd, sid -> si", seg_token_embeds, seg_image_token_embeds)  # [N_seg, 256]
        # return similarity

    def forward(self, hidden_states, input_ids, seg_token_mask, latent_token_num, num_patches, **kwargs):

        # reset per-call buffers for REINFORCE
        self.log_probs = None
        if self.mode == 1:
            return self.quick_select(input_ids, hidden_states, seg_token_mask, num_patches, latent_token_num)
        self.seg_token_embeds, self.seg_image_token_embeds, self.latent_token_embeds = self.recurrent_unrolled(hidden_states, input_ids, seg_token_mask, latent_token_num, num_patches)
    ...
    def _policy_walker_mode1(self):
        ...
        latent_token_embeds = self.latent_token_embeds[batch_indices, selected_indices]
        return seg_token_embeds_for_similarity, seg_image_token_embeds_for_similarity, seg_token_embeds_for_similarity, latent_token_embeds
    ...
    def _policy_walker_mode2(self):
        ...
        latent_token_embeds = self.latent_token_embeds[batch_indices, selected_indices]
        return seg_token_embeds_for_similarity, seg_image_token_embeds_for_similarity, seg_token_embeds_for_sam, latent_token_embeds
    ...
    def _policy_walker_mode3(self):
        ...
        latent_token_embeds = self.latent_token_embeds[batch_indices, actions]
        return seg_token_embeds_for_similarity, seg_image_token_embeds_for_similarity, seg_token_embeds_for_sam, latent_token_embeds
    ...
    def _policy_walker_mode4(self):
        ...
        self.log_probs = None
        latent_token_embeds = torch.einsum('nl,nlkd->nkd', layer_weights, self.latent_token_embeds)
        return seg_token_embeds_for_similarity, seg_image_token_embeds_for_similarity, seg_token_embeds_for_sam, latent_token_embeds
