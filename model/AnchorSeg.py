...
from model.PPMv2 import PolicyPromptedMasking
...
class AnchorSegMetaModel(nn.Module):
    def __init__(
        self,
        config,
        **kwargs,
    ...
    def initialize_decoder_modules(self, config):
        ...
        if self.local_rank == 0 and self.logger is not None:
            self.logger.info("Initializing AnchorSeg decoder modules...")
        
        result = self._initialize_AnchorSeg_modules(config)
    ...
    def _initialize_AnchorSeg_modules(self, config):
        # SAM
        ...
        self.latent_hidden_fcs = nn.ModuleList([nn.Sequential(*latent_fc)])
        self.latent_hidden_fcs.train()
        for param in self.latent_hidden_fcs.parameters():
            param.requires_grad = True


class AnchorSegModel(AnchorSegMetaModel, LlavaLlamaModel):
    def __init__(
        self,
        config,
        **kwargs,
    ):
        super(AnchorSegModel, self).__init__(config, **kwargs)
        # self.config = config
...
class AnchorSegForCausalLM(LlavaLlamaForCausalLM):
    def __init__(
        self,
        config,
        **kwargs,
        ...
        self.latent_token_idx = kwargs.pop("latent_token_idx")
        self.seg_token_num = kwargs.pop("seg_token_num", 1)
        self.latent_token_num = kwargs.pop("latent_token_num", 32)
        super(AnchorSegForCausalLM, self).__init__(config)
        self.model = AnchorSegModel(config, **kwargs)
        ...
        self.Token_Mask_Cycle_Consistency = []
        self.Mask_Token_Cycle_Consistency = []
    ...
    def generate_pred_masks(self, pred_embeddings, latent_embeddings, image_embeddings, sam_mask_shape_list, similarity):
        ...
        for i in range(len(pred_embeddings)):
            ...
            sparse_embeddings, dense_embeddings = self.model.visual_model.prompt_encoder(
                points=None, boxes=None, masks=similarity_map, text_embeds=torch.cat([latent_embeddings[i], pred_embeddings[i].unsqueeze(1)], dim=1)
            )
            
            sparse_embeddings = sparse_embeddings.to(pred_embeddings[i].dtype)
            spatial_masking = F.interpolate(similarity_map.float(), size=(64, 64), mode="nearest").to(torch.bfloat16)
    ...
    def model_forward(
        ...
        **kwargs,
        ...
        # latent_ids = torch.as_tensor(self.latent_token_idx, device=input_ids.device, dtype=input_ids.dtype)
        # latent_token_mask = torch.isin(input_ids, latent_ids)
        # labels[latent_token_mask] = IGNORE_INDEX 
        self.Token_Mask_Cycle_Consistency.clear()
        self.Mask_Token_Cycle_Consistency.clear()
        ...
        seg_token_embeds_for_similarity, seg_image_token_embeds_for_similarity, \
            seg_token_embeds_for_sam, latent_token_embeds = self.ppm(output.all_hidden_states, input_ids, seg_token_mask, self.latent_token_num, num_tokens_per_image, **kwargs)
        
        pred_embeddings = self.model.text_hidden_fcs[0](seg_token_embeds_for_sam)
        latent_embeddings = self.model.latent_hidden_fcs[0](latent_token_embeds)
        ...
        for i in range(len(seg_token_offset) - 1):
            start_i, end_i = seg_token_offset[i], seg_token_offset[i + 1]
            latent_embeddings_.append(latent_embeddings[start_i:end_i])
        latent_embeddings = latent_embeddings_
        ...
        pred_masks = self.generate_pred_masks(pred_embeddings, latent_embeddings, image_embeddings, sam_mask_shape_list, similarity)
        ...
        for batch_idx in range(len(pred_masks)):
            ...
            gaussian_bce_loss = (self.simi_loss.compute_gaussian_bce_loss(
                self.Token_Mask_Cycle_Consistency[batch_idx], gt_mask, num_masks=gt_mask.shape[0]) 
                * gt_mask.shape[0] 
            )
            ...
            gt_mask_ = self.apply_image_torch(gt_mask.unsqueeze(1), target_length=336)
            gt_mask_ = F.interpolate(gt_mask_.float(), size=(24, 24), mode="nearest").squeeze(1)
            gt_mask_ = self.simi_loss.gaussian_heatmap_from_mask(gt_mask_).to(torch.bfloat16)
            pred_mask_ = self.Mask_Token_Cycle_Consistency[batch_idx].reshape(gt_mask.shape[0], 24, 24)
            mask_bce_loss += (
                sigmoid_ce_loss(
                    pred_mask_, 
                    gt_mask_, 
    ...
    def get_similarity_map(self, sm, shape, target_length = 336):
        ...
        self.Mask_Token_Cycle_Consistency.append(sm)
        # reshape
        ...
        self.Token_Mask_Cycle_Consistency.append(sm.squeeze(1))
        # import cv2
        # import os
        # import numpy as np
        # layer_name = 'AnchorSeg-7B@reason_seg_train/layer40'
    ...
    def evaluate(
        ...
        **kwargs
        ...
        with torch.inference_mode():
            ...
            seg_token_embeds_for_similarity, seg_image_token_embeds_for_similarity, \
            seg_token_embeds_for_sam, latent_token_embeds = self.ppm(self.all_hidden_states, input_ids, seg_token_mask, self.latent_token_num, num_tokens_per_image, **kwargs)
            pred_embeddings = self.model.text_hidden_fcs[0](seg_token_embeds_for_sam)
            latent_embeddings = self.model.latent_hidden_fcs[0](latent_token_embeds)
        ...
        **kwargs
        ...
        with torch.no_grad():
            ...
            for idx, input_id in enumerate(input_ids):
                ...
                assert len(self.model.text_hidden_fcs) == 1
                # seg_token_embeds_for_similarity, seg_image_token_embeds_for_similarity, \
                # seg_token_embeds_for_sam = self.ppm(self.all_hidden_states, _input_id[None], seg_token_mask, self.latent_token_num, num_tokens_per_image, **kwargs)
                # pred_embeddings = self.model.text_hidden_fcs[0](seg_token_embeds_for_sam)
                seg_token_embeds_for_similarity, seg_image_token_embeds_for_similarity, \
                seg_token_embeds_for_sam, latent_token_embeds = self.ppm(self.all_hidden_states, _input_id[None], seg_token_mask, self.latent_token_num, num_tokens_per_image, **kwargs)
                pred_embeddings = self.model.text_hidden_fcs[0](seg_token_embeds_for_sam)
                latent_embeddings = self.model.latent_hidden_fcs[0](latent_token_embeds)
                ...
                all_latent_embeddings.extend([latent_embeddings])
            ...
            latent_embeddings = [torch.cat(all_latent_embeddings, dim=0)]
...
def load_pretrained_model_AnchorSeg(
    model_path,
    device_map="auto",
    device="cuda",
    **kwargs,
    ...
    model = AnchorSegForCausalLM.from_pretrained(
...
def init_AnchorSeg_model(args, model_args):
    # Create model
    ...
    model.get_model().initialize_AnchorSeg_modules(model.get_model().config)
