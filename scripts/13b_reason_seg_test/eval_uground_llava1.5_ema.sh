...
deepspeed --include "localhost:7" --master_port="$MASTER_PORT" train_ds.py \
  --model_key="$MODEL_KEY" \
  --version="./runs/UGround-13b_reason_seg_test_llava1.5_ema/hf-UGround-13b_reason_seg_test_llava1.5_ema" \
  --dataset_dir='../dataset_sesame' \
  --dataset="vqa" \
  --sample_rates="1" \
  --exp_name="UGround-13b_reason_seg_test_llava1.5_ema/eval" \
