...
deepspeed --include "localhost:0" --master_port="$MASTER_PORT" train_ds.py \
  --model_key="$MODEL_KEY" \
  --version="./runs/UGround-7b_multi_reason_seg_llava1.5_ema/hf-UGround-7b_multi_reason_seg_llava1.5_ema" \
  --dataset_dir='../dataset_sesame' \
  --dataset="vqa" \
  --sample_rates="1" \
  --exp_name="UGround-7b_multi_reason_seg_llava1.5_ema/eval" \
