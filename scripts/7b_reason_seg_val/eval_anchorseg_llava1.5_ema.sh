...
deepspeed --include "localhost:5" --master_port="$MASTER_PORT" train_ds.py \
  --model_key="$MODEL_KEY" \
  --version="runs/AnchorSeg-7b_reason_seg_val_llava1.5_ema/hf-AnchorSeg-7b_reason_seg_val_llava1.5_ema" \
  --dataset_dir='../dataset_sesame' \
  --dataset="vqa" \
  --sample_rates="1" \
  --exp_name="AnchorSeg-7b_reason_seg_val_llava1.5_ema/eval" \
