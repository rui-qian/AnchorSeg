...
deepspeed --master_port="$MASTER_PORT" --include "localhost:2,3,4,5" train_ds.py \
  --model_key="$MODEL_KEY" \
  --version="../dataset_sesame/llava-v1.5-7b" \
  --dataset_dir='../dataset_sesame' \
  --exp_name="UGround-7b_refer_seg_llava1.5_ema" \
  ...
  --resume_from_best \
  --dataset="refer_seg||sem_seg||neg_refer_seg||correct_refer_seg||vqa||reason_seg||reason_seg_plus||multi_reason_seg" \
  --sample_rates="12,0,12,0,0,0,0,0" \
