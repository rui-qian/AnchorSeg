...
from .qa_template import SHORT_QUESTION_TEMPLATE, LONG_QUESTION_TEMPLATE, LONG_ANSWER_TEMPLATE, SHORT_ANSWER_TEMPLATE
...
class ReasonSegDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        if use_expand_question_list:
            ...
            self.answer_list.extend(LONG_ANSWER_TEMPLATE)
        ...
        print("number of reason_seg samples(train split): ", len(images))
    ...
    def sample_answer_template(self, answer_list, predicate=None):
        candidates = (
            [t for t in answer_list if predicate(t)]
    ...
    def __getitem__(self, idx):
        ...
        questions = []
        answers = []
        seg_token = "[SEG]" if self.seg_token_num == 1 else ' '.join(["[SEG{}]".format(i) for i in range(self.seg_token_num)])
        ...
        for text in sampled_sents:
            ...
            if self.explanatory != -1 and img_name in self.img_to_explanation:
                if choice == 0:  # [SEG]
                    predicate = ((lambda t: "{class_name}" not in t) if is_sentence else None)
                    answer_temp = self.sample_answer_template(self.answer_list, predicate)
                    answer_temp = answer_temp.replace('[SEG]', seg_token) if self.seg_token_num > 1 else answer_temp
                    answer_temp = answer_temp.format(class_name=text.lower()) if "{class_name}" in answer_temp else answer_temp
        ...
        # Generate conversations after all questions and answers are prepared
        conversations = []
...
if __name__ == "__main__":
    ...
    for batch_idx, batch_data in enumerate(dataloader):
        ...
        for i, (conversation, masks) in enumerate(zip(batch_data['conversations'], batch_data['masks'])):
            ...
            # if not check1:
            print(batch_idx,batch_data['image_paths'][i])
