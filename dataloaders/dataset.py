...
from .fp_reason_seg_dataset import fpReasonSegDataset
from .fp_refer_seg_dataset import fpReferSegDataset
from .fp_sem_seg_dataset import fpSemSegDataset
...
def collate_fn(
    ...
    if LATENT_PLACEHOLDER_STR:
        for i in range(len(conversation_list)):
            if "[SEG]" in conversation_list[i]:
                conversation_list[i] = conversation_list[i].replace("[SEG]", LATENT_PLACEHOLDER_STR + " " + "[SEG]")
...
class HybridDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        for dataset in self.datasets:
            ...
            elif dataset == "fp_refer_seg":
                def filter_datasets(s, exclude):
                    return "||".join(x for x in s.split("||") if x not in exclude)
                exclude = {"refzom", "grefcoco"}
                refer_seg_data = filter_datasets(refer_seg_data, exclude)
                self.all_datasets.append(
                    fpReferSegDataset(
                        base_image_dir,
                        tokenizer,
...
class ValDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.multi_refer_seg = True # set False to use one-question-per-object setting
    ...
    def __getitem__(self, idx):
        ...
        if self.data_type == 'refer_seg' and self.multi_refer_seg:
            conv.messages = []
            ...
            while i < len(sampled_sents):
                ...
                if is_sentence:
                    ...
                    # conv.append_message(conv.roles[1], "Sure, it is {}.".format(_seg))
                else:
                    conv.append_message(
                        conv.roles[0],
                        DEFAULT_IMAGE_TOKEN
                    ...
                    if self.multiseg_inference:
                        ...
                        conv.append_message(conv.roles[1], "{}.".format(_seg))
                        # conv.append_message(conv.roles[1], "Sure, {class_name}:{seg}.".format(class_name=text, seg=_seg))
                conversations.append(conv.get_prompt())
                if self.multiseg_inference:
                    i += sample_num
...
def collate_fn_val(batch, tokenizer=None, use_mm_start_end=True, padding="right", 
    ...
    # Replace "[SEG]" with HiMTok placeholders so the LLM can predict mask tokens.
    # Note: we intentionally keep "[REJ]" unchanged (no-target cases).
    if LATENT_PLACEHOLDER_STR:
        for i in range(len(conversation_list)):
            if "[SEG]" in conversation_list[i]:
                conversation_list[i] = conversation_list[i].replace("[SEG]", LATENT_PLACEHOLDER_STR + " " + "[SEG]")
...
# class TestReferDataset(NegReferSegDataset):
